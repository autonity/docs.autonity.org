...
- To generate the `proof` of enode ownership required for validator registration. The `proof` is generated using the server key and the node's [p2p node key](/concepts/validator/#p2p-node-key) using the [`genOwnershipProof`](/reference/cli/agc/#command-line-options) command-line option of the Autonity Go Client. 
