...
| `symbols` | the currency pairs that the oracle network provides data points for | See [`config.autonity.symbols`](/reference/genesis/#configautonityoracle-object) | None |
| `votePeriod` | the time interval at which the oracle network initiates a new oracle round for submitting and voting on oracle data, measured in blocks | See [`config.autonity.votePeriod`](/reference/genesis/#configautonityoracle-object) | None |
