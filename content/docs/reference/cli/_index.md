...
For `aut` installation, usage, and command-line options see Reference [Setup the Autonity Utility Tool (aut)](/account-holders/setup-autcli/).
...
| --| --|
| COMMANDS: ||
| `dumpgenesis` | The command is removed and has been replaced by an Autonity Protocol contract `[config](/reference/api/aut/#config)` method to return the genesis block  configuration |
...
| `miner.gaslimit` | The default value for Ethereum's native gas block limit voting mechanism is changed from '8000000' to '30000000', the EIP-1559 block gas limit |
