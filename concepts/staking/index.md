...
Redemption of Liquid Newton for Newton stake token is by unbonding. The Liquid Newton holder can unbond stake from a validator and redeem Newton at any time subject to the unbonding period set for the chain. On unbonding, the unbonded Liquid Newton stake is locked and enters an [unbonding period](/glossary/#unbonding-period). The unbonding Liquid Newton stake remains subject to any [slashing penalties](/glossary/#slashing-penalty) applied to the validator between unbonding and stake redemption, which takes place at the end of the epoch in which the unbonding period expires. At this point the validator's stake pool is reduced by the unbonded amount, the locked Liquid Newton is  burned, and Newton is minted to the staker.
...
Stake is unbonded from a validator through an unbonding operation. Unbonding is subject to an [unbonding period](/concepts/staking/#unbondingperiod) during which it is in an `unbonding` state and cannot be transferred. The unbonding period applies irrespective of whether the nominated validator is a member of the consensus committee or not. 
...
Alice has [delegated](/glossary/#delegated) stake to a validator. She submits an `unbond()` tx to unbond 100 LNTN that is processed and included in a block at time `T`:
