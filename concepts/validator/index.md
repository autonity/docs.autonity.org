...
- Provide bonded stake to the Proof of Stake consensus mechanism, providing bonded stake for network security economics.
...
In the case of unbonding: if the unbonding request is included in block `T`, then actual unbonding is then executed at `T` + [`unbondingPeriod`](/concepts/staking/#unbondingperiod) + remainder of the [`epoch`](/concepts/staking/#epoch) in which the unbonding period falls. At this point, the staker's due newton is redeemed and minted to them. The redemption flow varies depending on whether the unbonding is for [self-bonded](/glossary/#self-bonded) or [delegated](/glossary/#delegated) stake:

- For [self-bonded](/glossary/#self-bonded) stake, the unbonding request amount is stated in NTN and that amount of newton is redeemed at epoch end.
- For [delegated](/glossary/#delegated) stake, the unbonding request amount is stated in LNTN and the amount of newton redeemed at epoch end depends on the NTN-LNTN conversion rate for the validator (see [Staking, Liquid Newton](/concepts/staking/#liquid-newton)). Liquid newton for [delegated](/glossary/#delegated) stake is burned immediately on processing of the [`unbond()`](/reference/api/aut/#unbond) function call. The validator's total bonded stake amount is then updated at `T` + [`unbondingPeriod`](/concepts/staking/#unbondingperiod) + remainder of the [`epoch`](/concepts/staking/#epoch) in which the unbonding period falls. At this point, the staker's due newton is redeemed and minted to them.
