...
This section describes the Autonity omission fault detection protocol, the role of validators in submitting and verifying omission accountability event proofs (fault, accusation, innocence), the lifecycle for omission fault accountability and detection, and slashing.
...
| There is not a _pending accusation_ | The _offending validator_ does not already have an _accusation_ for committing a rule infraction in the epoch. If there is a _pending accusation_, then the accountability protocol has to wait until the _innocence proof submission window_ for the _pending accusation_ expires to determine whether a _new accusation_ can be made or not. This creates a _deadline_ measured in block height *before* it becomes possible to submit a new _accusation_. The _innocence proof submission window_ length is set in the [slashing protocol configuration](/concepts/ofd/#slashing-protocol-configuration). |
...
An _innocence_ claim must be submitted within an _innocence proof submission window_ to be accepted. The window length is measured as a count of blocks set as a parameter in the [slashing protocol configuration](/concepts/ofd/#slashing-protocol-configuration). The window begins at the block height number at which the _accusation_ was reported and gives the accused validator a window of opportunity to detect if an accusation against it has been made, and if so to defend itself and prove innocence by submitting an _innocence_ claim on-chain. Otherwise, the _accusation_ may be _promoted_ to a _fault_ and become slashable at the end of epoch.
...
The slashing amount is calculated from a number of parameters, including _severity_. For slashing calculation and parameters see [Slashing](/concepts/ofd/#slashing). For the severity taxonomy see [Rule severity](/concepts/ofd/#rule-severity).
...
- `base rate`: is determined by the [severity](/concepts/ofd/#rule-severity) of the rule infraction
...
The _slashing amount_ is calculated by the formula `(slashing rate * validator bonded stake)/slashing rate scale factor`, applying the computed _slashing rate_ as a multiplier to the validator's bonded stake amount divided by the [slashing protocol configuration](/concepts/ofd/#slashing-protocol-configuration) _slashing rate scale factor_.
...
- `jail factor`: A multiplier measured as a number of epochs, defined as a protocol parameter in the [slashing protocol configuration](/concepts/ofd/#slashing-protocol-configuration).
...
::: {.callout-note title="Note" collapse="false"}
See distinction between _direct_ and _promoted_ in [Faults](/concepts/ofd/#faults) above.
:::
...
::: {.callout-note title="Note" collapse="false"}
Note the validator can determine if a detected fault is accusable by calling the [`canAccuse()`](/reference/api/ofd/#canaccuse) and [`canSlash()`](/reference/api/ofd/#canslash) contract functions.
:::
-->
